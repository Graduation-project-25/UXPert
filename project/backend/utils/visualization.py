...
import pandas as pd
from backend.utils.ui_extraction import extract_ui_elements, save_ui_elements
...
def visualize_ui_elements(image_folder, json_folder, output_folder, limit=50):
    ...
    while True:
        ...
        for element in ui_elements:
            draw_bounding_box(element, image)
...
def draw_bounding_box(element, image):
...
def scatter_plot_ui_elements(df):
...
def clustering_visualization_by_position(DBSCAN_dataset, clusters):
    # Plot noise points (-1)
    if -1 in clusters: 
        # Assert 'Cluster' in DBSCAN_dataset.columns, "Column 'Cluster' not found in DBSCAN_dataset"
        plt.scatter(
...
def visualize_alignment_consistency(cluster_data):
...
def visualize_color_consistency(cluster_data):
    """
...
def visualize_size_proportionality(cluster_data):
    """
