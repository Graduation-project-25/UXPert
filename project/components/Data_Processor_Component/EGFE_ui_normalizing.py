...
class EGFE_UiNormalizing(UiNormalizerInterface):
    ...
    #     combined_elements_df = combined_elements_df.fillna(0)  # For numeric columns
    #     # combined_elements_df = combined_elements_df.dropna()


    #     # combined_screen_sizes_df = pd.concat(all_screen_sizes, ignore_index=True)

    #     return combined_elements_df
    ...
    #     # Initialize empty lists to store the normalized data and screen sizes for all files
    #     all_normalized_data = []
    #     # all_screen_sizes = []
    ...
    #         # Get normalized data and screen size
    #         normalized_elements, normalized_screen_size = self.get_normalized_data(data)
            
    #         # Add file name to the DataFrame for tracking (optional)
    #         # normalized_elements["file_name"] = json_file_path.split("/")[-1]  
    ...
    #     # Ensure that data has been processed
    #     if not all_normalized_data:
    #         raise ValueError("No valid JSON files were found or processed.")

    #     # Return the list of normalized elements and screen sizes
