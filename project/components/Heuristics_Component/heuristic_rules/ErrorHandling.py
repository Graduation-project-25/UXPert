    ...
    def check_error_messages(self, ui_data):
        ...
        for _, row in ui_data.iterrows():
            if row.get('type', '').strip().upper() == 'TEXT' and any(keyword in row.get('name', '').lower() for keyword in error_keywords):
                ...
                # Add coordinates to the issue
                x = row.get('x', 'Unknown')  # Assuming 'x' and 'y' are part of your dataset
                y = row.get('y', 'Unknown')
                width = row.get('width', 'Unknown')
                height = row.get('height', 'Unknown')

                if not text_content:
                    issues.append(f"Error message '{row['name']}' is present but empty at position ({x}, {y}).")
                elif len(text_content.split()) < 3:
                    issues.append(f"Error message '{row['name']}' is too short and unclear at position ({x}, {y}).")
                
                if not any(kw in style for kw in ['red', 'bold', 'alert', 'warning']):
                    issues.append(f"Error message '{row['name']}' may not be visually distinguishable at position ({x}, {y}).")

                if not any(kw in text_content.lower() for kw in guidance_keywords):
                    issues.append(f"Error message '{row['name']}' lacks guidance on how to fix the issue at position ({x}, {y}).")
        ...
        for _, row in ui_data.iterrows():
            if row.get('type', '').strip().upper() in ['BUTTON', 'LINK']:
                ...
                if any(keyword in name for keyword in recovery_keywords):
                    ...
                    issues.append(f"Button '{row['name']}' may not be clear at position ({x}, {y}). Consider using a more descriptive label.")
    ...
    def check_error_placement(self, ui_data):
        ...
        for _, row in ui_data.iterrows():
            if row.get('type', '').strip().upper() == 'TEXT' and 'error' in row.get('name', '').lower():
                ...
                if related_id and related_id not in input_fields:
                    issues.append(f"Error message '{row['name']}' may not be properly linked to an input field at position ({x}, {y}).")
