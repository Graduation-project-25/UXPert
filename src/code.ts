...
figma.ui.onmessage = async (msg) => {
    if (msg.type === 'start-detection') {
      ...
      try {
        ...
        const responseJson = await processResponse.json() as { feedback: string };
  
        const feedback = responseJson.feedback || "No feedback available";  // Assuming backend sends feedback
